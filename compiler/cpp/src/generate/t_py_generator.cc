...
void t_py_generator::generate_service_client(t_service* tservice) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    f_service_ <<
      indent() << "import sys" << endl <<
      indent() << "import datetime" << endl <<
      indent() << "print \"%s client.send %s.%s\" %(datetime.datetime.now(), self.__class__.__name__, sys._getframe().f_code.co_name)" << endl;
    ...
    if (!(*f_iter)->is_oneway()) {
      ...
      f_service_ <<
        indent() << "import sys" << endl <<
        indent() << "import datetime" << endl <<
        indent() << "print \"%s client.recv %s.%s\" %(datetime.datetime.now(), self.__class__.__name__, sys._getframe().f_code.co_name)" << endl;
  ...
  if (gen_twisted_) {
    ...
    */
    ...
    f_service_ <<
      indent() << "import sys" << endl <<
      indent() << "import datetime" << endl <<
      indent() << "print \"%s server.recv %s.%s\" %(datetime.datetime.now(), self.__class__.__name__, sys._getframe().f_code.co_name)" << endl;
    ...
    f_service_ <<
      indent() << "import sys" << endl <<
      indent() << "import datetime" << endl <<
      indent() << "print \"%s server.send %s.%s\" %(datetime.datetime.now(), self.__class__.__name__, sys._getframe().f_code.co_name)" << endl;
